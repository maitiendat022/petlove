                    ...
                    <div class="col-12 mt-3">
                        <div class="card-body">
                            <form action="{{ route('categories.update',$category->id) }}" method="POST" enctype="multipart/form-data">
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="pet_id" id="pet_id">
                                                        ...
                                                        <option value="{{ $item->id }}"{{ (old('pet_id') == $item->id ||  $category->pet_id == $item->id) ? 'selected' : '' }}>{{ $item->name }}</option>
                                                        @endforeach
                                                    </select>
                                                    @error('pet_id')
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="parent_id" id="parent_id">
                                                        ...
                                                        @if($item->id != $category->id)
                                                            <option data-pet="{{ $item->pet_id }}" value="{{ $item->id }}"{{ (old('parent_id') == $item->id || $category->parent_id == $item->id) ? 'selected' : '' }} >{{
                                                                $item->name }}
                                                            </option>
                                                        @endif
                                                        @endforeach
                                ...
                                <button type="submit" class="btn btn-info btn-fill mt-2 mb-4">Cập nhật</button>
